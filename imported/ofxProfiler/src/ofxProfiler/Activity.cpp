...
namespace ofxProfiler {
	...
	void Activity::begin() {
		if (this->state == Active) {
			//std::cout << "ofxProfiler::Activity::begin" << "Cannot begin activity [" << this->name << "] as it is already active." << std::endl;
			return;
		}
	...
	void Activity::end() {
		...
		if (this->state == Waiting) {
			//std::cout << "ofxProfiler::Activity::end" << "Cannot end activity [" << this->name << "] as it is not active."<< std::endl;
			return;
		}
		if (now < this->beginTime) {
			std::cout << "ofxProfiler::Activity::end" << "Activity ended before it began" << std::endl;
			return;
		}
	...
	*/
	...
	string Activity::getResults() {
		if (this == & Activity::Root()) {
			...
			if (this->state == Active) {
				std::cout << "ofxProfiler::Activity::getResults()" << std::endl << "[" << this->name << "] Cannot print results whilst active. Please call end before getResults";
				return "";
			}
